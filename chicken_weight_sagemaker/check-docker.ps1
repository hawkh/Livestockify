...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "${Green}✅ Docker is working correctly${Reset}"
    } else {
        Write-Host "${Yellow}⚠️  Docker test failed, but daemon is running${Reset}"
    }
} catch {
    Write-Host "${Yellow}⚠️  Could not test Docker functionality${Reset}"
}
